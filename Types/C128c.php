...
/**
 * Code 128c - yoğun, yüksek güvenlikli; dünya çapında çok yaygın kullanımdaki tür.
 * Code 128c - Very capable code, excellent density, high reliability; in very wide use world-wide.
 */
class C128c extends BarcodeGenerator
{
    ...
    /**
     * C128c standartında kod dizesi oluşturur.
     * Generates code string in C128c standard.
     */
    protected function generateCodeString(): string
    {
        $keys       = \array_keys(self::$CONVERSION_TABLE);
        ...
        for ($pos = 1; $pos <= $length; $pos += 2) {
            ...
            $codeStr .= self::$CONVERSION_TABLE[$activeKey];
            $checksum += $values[$activeKey] * $checkPos;
            ++$checkPos;
        }

        return $codeStr . self::$CONVERSION_TABLE[$keys[($checksum - ((int) ($checksum / 103) * 103))]];
    }
}